	...
	CREATE OR REPLACE FUNCTION public.kapi_tree_structure_new_nodes(
        ...
        BEGIN
          ...
          CREATE TABLE IF NOT EXISTS ' || _table_name_full || '(
                node_id  NOT NULL DEFAULT uuid_generate_v4(),
            ...
            node_depth bigint GENERATED ALWAYS AS (nlevel(node_path)::bigint) STORED,
                ...
                node_link_metadata default {}::jsonb,
                node_link_data default {}:jsonb,
                -- Date and time ------------------------------------
                node_inserted_at NOT NULL DEFAULT ((date_part(epoch::text, CURRENT_TIMESTAMP AT TIME ZONE $3) * (1000)::double precision))::bigint
                CHECK (char_length(VALUE::text) = 13),
                node_updated_at NOT NULL DEFAULT ((date_part(epoch::text, CURRENT_TIMESTAMP AT TIME ZONE $3) * (1000)::double precision))::bigint
                CHECK (char_length(VALUE::text) = 13),
                -- CONSTRAINTS --------------------------------------
        ...
        BEGIN
          ...
          CREATE TABLE IF NOT EXISTS ' || _table_name_full || '(
            ...
            data_note kapi_dtd_text_null_or_notempty,
                ...
                data_inserted_at NOT NULL DEFAULT ((date_part(epoch::text, CURRENT_TIMESTAMP AT TIME ZONE $3) * (1000)::double precision))::bigint
                CHECK (char_length(VALUE::text) = 13),
                data_updated_at NOT NULL DEFAULT ((date_part(epoch::text, CURRENT_TIMESTAMP AT TIME ZONE $3) * (1000)::double precision))::bigint
                CHECK (char_length(VALUE::text) = 13)
            );
