defmodule Prueba.Repo.Migrations.InstallKapi do
  use Ecto.Migration
  ...
  def change do
        ...
        #-- Edit  by  Francisco Javier Cortez
        #
        ...
        execute """
        DROP DOMAIN IF EXISTS public.kapi_dtd_epoch_auto cascade;
        """,""
        execute """
        ...
        #---------------------------------------------------------------
        execute """
        DROP DOMAIN IF EXISTS public.kapi_dtd_uuid cascade;
        """,""
        execute """
        ...
        execute """
        DROP DOMAIN IF EXISTS public.kapi_dtd_uuid_auto cascade;
        """,""
        execute """
        ...
        #---------------------------------------------------------------
        execute """
        DROP DOMAIN IF EXISTS public.kapi_dtd_citext CASCADE;
        """,""
        ...
        execute """
        DROP DOMAIN IF EXISTS public.kapi_dtd_citext_default CASCADE;
        """,""
        ...
        #-- '             a a          '  Error
        execute """
        DROP DOMAIN IF EXISTS public.kapi_dtd_citext_notempty CASCADE;
        """,""
        execute """
        ...
        """,""
        execute """
        --INSERT INTO locations.location_groups(room_id, key, name, state, description, extras)
	  --VALUES ('96c508c1-407c-4f08-8410-06ad9ed6a92d', 'almancen1', 'almacen1', 'active', 'Lugar de almacenaje', '{}');
      ...
      BEGIN
        ...
        CREATE TABLE IF NOT EXISTS ' || _table_name_full || '(
          ...
          node_depth bigint GENERATED ALWAYS AS (nlevel(node_path)::bigint) STORED,
              ...
              node_link_weight INTEGER default 0,
              node_link_metadata jsonb default ''' || '{}' ||'''::jsonb,
              node_link_data  jsonb default ''' || '{}' ||'''::jsonb,
              -- Date and time ------------------------------------
              node_inserted_at bigint NOT NULL DEFAULT ((date_part( ''' || 'epoch' || '''::text, CURRENT_TIMESTAMP AT TIME ZONE ''' || 'UTC' || ''' ) * (1000)::double precision))::bigint,
              node_updated_at bigint NOT NULL DEFAULT ((date_part( ''' || 'epoch' || '''::text, CURRENT_TIMESTAMP AT TIME ZONE ''' || 'UTC' || ''' ) * (1000)::double precision))::bigint,
              -- CONSTRAINTS --------------------------------------
          CONSTRAINT _uk_group_parent_key_' || _table_name || ' UNIQUE (node_group_id, node_path_to, node_key),
          CONSTRAINT _uk_node_group_id_room_id_' || _table_name || ' UNIQUE (node_group_id, room_id)
          );
          ...
          CREATE INDEX IF NOT EXISTS _idx_room_id_' || _table_name || ' ON ' || _table_name_full || ' (room_id);
          ...
          CREATE INDEX IF NOT EXISTS _idx_group_room_id_' || _table_name || ' ON ' || _table_name_full || ' (node_group_id, room_id);
    ...
    LANGUAGE plpgsql;
        ...
        BEGIN
          ...
          CREATE TABLE IF NOT EXISTS ' || _table_name_full || '(
                ...
                data_inserted_at bigint NOT NULL DEFAULT ((date_part( '''|| 'epoch' ||'''::text, CURRENT_TIMESTAMP AT TIME ZONE '''|| 'UTC' ||''' ) * (1000)::double precision))::bigint,
                data_updated_at bigint NOT NULL DEFAULT ((date_part( '''|| 'epoch' ||'''::text, CURRENT_TIMESTAMP AT TIME ZONE '''|| 'UTC' ||''' ) * (1000)::double precision))::bigint

            );
            CREATE INDEX IF NOT EXISTS _idx_data_node_id_room_id_' || _table_name || ' ON ' || _table_name_full || ' (data_node_id, room_id);
       ...
       CREATE OR REPLACE FUNCTION public.kapi_tree_structure_new_tree(
        ...
        BEGIN
          ...
          CREATE SCHEMA IF NOT EXISTS ' || _schema || ';
            ...
            CREATE UNIQUE INDEX IF NOT EXISTS _uk_group_room_id_' || _table_name || ' ON ' || _table_name_full || ' (node_group_id, room_id);
          ...
          CREATE INDEX IF NOT EXISTS _idx_group_' || _table_name || ' ON ' || _table_name_full || ' (node_group_id);
            ...
            CREATE INDEX IF NOT EXISTS _idx_node_updated_at_' || _table_name || ' ON ' || _table_name_full || ' (node_updated_at);
            CREATE INDEX IF NOT EXISTS _idx_group_node_key_' || _table_name || ' ON ' || _table_name_full || ' (node_group_id, node_key);
            CREATE INDEX IF NOT EXISTS _idx_group_node_room_id_' || _table_name || ' ON ' || _table_name_full || ' (node_group_id, room_id);
            CREATE INDEX IF NOT EXISTS _idx_group_node_alias_' || _table_name || ' ON ' || _table_name_full || ' (node_group_id, node_alias);
            CREATE INDEX IF NOT EXISTS _idx_node_key_node_alias_fst_' || _table_name || ' ON ' || _table_name_full || ' USING gist (node_key, node_alias);
            CREATE INDEX IF NOT EXISTS _idx_node_key_node_alias_path_fst_' || _table_name || ' ON ' || _table_name_full || ' USING gist (node_key, node_alias, node_path);
            -- View indexes
